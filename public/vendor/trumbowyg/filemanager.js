/*!
 * Trumbowyg File Manager — plugin custom NestAdmin.
 * Tombol toolbar "filemanager" membuka modal: daftar gambar dari OSS
 * (GET /admin/v1/media/list), upload baru (POST .../upload), hapus
 * (POST .../delete), dan sisip <img> ke editor saat thumbnail diklik.
 *
 * CSRF: token diambil dari <meta name="csrf-token"> dan dikirim via header
 * x-csrf-token (di-handle middleware csrf core).
 *
 * Memerlukan jQuery + Trumbowyg (sudah dimuat di head layout). Modal pakai
 * markup + style sendiri (tanpa Bootstrap JS) agar lepas dependensi.
 */
(function ($) {
    'use strict'
    if (!$ || !$.trumbowyg) return
