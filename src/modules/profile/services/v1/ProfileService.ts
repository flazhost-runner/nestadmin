...
export class ProfileService implements IProfileService {
  ...
  async update(
    ...
    // Handle file upload — buffer multer (memoryStorage) → StorageService,
    // jalan di semua STORAGE_DRIVER (local/oss/s3). Lihat StorageService.saveUpload.
    const key = await this.storage.saveUpload(
      files?.picture?.[0],
      'uploads/profile',
    );
    if (key) request.picture = key;
